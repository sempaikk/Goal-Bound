...
const { renderCatalogPage, CARDS_PER_PAGE_CATALOG } = require('../services/CollectionCatalogRenderer.js');
const {
  TIER_ALL,
  TIER_COACH,
  TIER_OPTIONS,
...
const CARDS_PER_PAGE = CARDS_PER_PAGE_CATALOG || 5;
...
async function buildCollectionContainer(opts) {
  ...
  let catalogAttachment = null;
  if (pageEntries.length > 0) {
    try {
      const catalogRows = pageEntries.map((e, i) => ({
        card: e.card,
        level: e.level,
        index: start + i + 1,
        onTeam: e.onTeam
      }));
      const buf = await renderCatalogPage(catalogRows, {
        page: safePage,
        totalPages,
        total: entries.length,
        username
      });
      catalogAttachment = new AttachmentBuilder(buf, { name: 'catalog.png' });
    } catch (err) {
      logger.warn('catalog render failed', err.message);
    }
  }
  ...
  if (catalogAttachment) {
    container.addMediaGalleryComponents(
      new MediaGalleryBuilder().addItems(
        new MediaGalleryItemBuilder().setURL('attachment://catalog.png')
      )
    );
  ...
  return { container, safePage, totalPages, catalogAttachment };
}
...
async function renderCollectionFor(viewerId, targetId, username, avatarURL, page, isSelf, sort = 'rarity', query = '') {
  ...
  const { container, safePage, totalPages, catalogAttachment } = await buildCollectionContainer({
...
async function replyCollectionV2(interaction, result, isEdit) {
  const payload = {
    ...
    files: result.catalogAttachment ? [result.catalogAttachment] : []
  };
