...
const { positionEmoji, emojiTag, safeTruncate, progressBar } = require('../utils/format.js');
...
function makeEntry(userCard, cards, rarity, cardIdsInTeam, rarityKey, userId) {
  ...
  const customEmoji = emojiTag(getEmojiForCard(card.id));
  const emoji = customEmoji || (isCoach ? '🎩' : rarity.emoji);
  ...
  return { rarityKey, line, name: card.name };
}

function formatPageDescription(pageEntries, sort, query, userId) {
  if (query || sort !== 'rarity') return pageEntries.map(e => e.line).join('\n');
  const blocks = [];
  let currentKey = null;
  let buffer = [];
  const flush = () => {
    if (buffer.length === 0 || !currentKey) return;
    if (currentKey === COACH_GROUP) blocks.push(`🎩 **${t(userId, 'profile_masters')}**\n${buffer.join('\n')}`);
    else {
      const rarity = RARITIES[currentKey];
      blocks.push(`${rarity.emoji} **${rarityLabel(userId, currentKey)}**\n${buffer.join('\n')}`);
    }
    buffer = [];
  };
  for (const entry of pageEntries) {
    if (entry.rarityKey !== currentKey) { flush(); currentKey = entry.rarityKey; }
...
function buildCollectionContainer(opts) {
  const {
  ...
  const listBody = pageEntries.length > 0
    ? formatPageDescription(pageEntries, sort, textQuery(query), viewerId)
    : (query
      ? t(viewerId, 'col_no_match')
      : t(viewerId, 'col_no_page'));
  const header =
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      ...
      new TextDisplayBuilder().setContent(safeTruncate(listBody || '—', 3800)),
  ...
  return { container, safePage, totalPages };
}

function renderCollectionFor(viewerId, targetId, username, avatarURL, page, isSelf, sort = 'rarity', query = '') {
  ...
  const { container, safePage, totalPages } = buildCollectionContainer({
    username, entries, page, cards, validUserCards, teamCount: teamRows.length, isSelf, sort, query, viewerId, targetId
  });
  return { container, totalCards: validUserCards.length, poolSize: cards.length, isEmpty: validUserCards.length === 0, safePage, totalPages };
}

async function replyCollectionV2(interaction, result, isEdit) {
  const payload = { components: [result.container], flags: MessageFlags.IsComponentsV2 };
...
async function openCollectionPanel(interaction, userId, username, avatarURL) {
  ...
  const result = renderCollectionFor(userId, userId, username, avatarURL, 0, true, 'rarity', '');
  if (result.isEmpty) {
...
async function openCollectionPanelForTarget(interaction, viewerId, targetUser) {
  ...
  const result = renderCollectionFor(viewerId, targetId, targetUser.username, targetUser.displayAvatarURL(), 0, isSelf, 'rarity', '');
  if (result.isEmpty) {
...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      const result = renderCollectionFor(viewerId, targetId, target.username, target.displayAvatarURL(), 0, isSelf, 'rarity', filterOpt);
      if (result.isEmpty) {
  ...
  async handleComponent(interaction) {
    ...
    if (action === 'pickuser') {
      ...
      try {
        ...
        const result = renderCollectionFor(viewerId, selected.id, selected.username, selected.displayAvatarURL(), 0, isSelf, 'rarity', '');
        if (result.isEmpty) {
    ...
    try {
      ...
      const result = renderCollectionFor(viewerId, targetId, username, avatarURL, nextPage, isSelf, sort, query);
      await replyCollectionV2(interaction, result, true);
    } catch (error) {
  ...
  async handleModal(interaction) {
    ...
    try {
      ...
      const result = renderCollectionFor(viewerId, targetId, username, avatarURL, 0, isSelf, sort, query);
      await replyCollectionV2(interaction, result, true);
    } catch (error) {
